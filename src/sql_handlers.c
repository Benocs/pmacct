...
void count_export_proto_sourceid_handler(const struct db_cache *cache_elem, struct insert_data *idata, int num, char **ptr_values, char **ptr_where)
{
  snprintf(*ptr_where, SPACELEFT(where_clause), where[num].string, cache_elem->primitives.export_proto_sourceid);
  snprintf(*ptr_values, SPACELEFT(values_clause), values[num].string, cache_elem->primitives.export_proto_sourceid);
  *ptr_where += strlen(*ptr_where);
  *ptr_values += strlen(*ptr_values);
}
