...
void write_stats_header_formatted(pm_cfgreg_t what_to_count, pm_cfgreg_t what_to_count_2, u_int8_t have_wtc, int is_event)
{
  if (!have_wtc) {
    ...
    if (what_to_count_2 & COUNT_EXPORT_PROTO_SOURCEID) printf("EXPORT_PROTO_SOURCEID  ");
...
void write_stats_header_csv(pm_cfgreg_t what_to_count, pm_cfgreg_t what_to_count_2, u_int8_t have_wtc, char *sep, int is_event)
{
  ...
  if (!have_wtc) {
    ...
    printf("%sEXPORT_PROTO_SOURCEID", write_sep(sep, &count));
    /* all custom primitives printed here */
    {
      char cp_str[SRVBUFLEN];
      int cp_idx;
    ...
    if (what_to_count_2 & COUNT_EXPORT_PROTO_SOURCEID) printf("%sEXPORT_PROTO_SOURCEID", write_sep(sep, &count));
  ...
  while (!errflag && ((cp = getopt(argc, argv, ARGS_PMACCT)) != -1)) {
	...
	if (!strcmp(count_token[count_index], "src_host")) {
	  ...
	  what_to_count |= COUNT_TAG;
        ...
        else if (!strcmp(count_token[count_index], "export_proto_sourceid")) {
          count_token_int[count_index] = COUNT_INT_EXPORT_PROTO_SOURCEID;
          what_to_count_2 |= COUNT_EXPORT_PROTO_SOURCEID;
        }
  ...
  if (want_counter || want_match) {
	...
	if (!strcmp(match_string_token, "*")) {
	  ...
	  request.pnat.timestamp_end.tv_usec = residual;
        ...
        else if (!strcmp(count_token[match_string_index], "export_proto_sourceid")) {
          char *endptr;

          request.data.export_proto_sourceid = strtoul(match_string_token, &endptr, 10);
        }
  ...
  if (want_stats || want_match) {
	...
	if (!have_wtc || (what_to_count_2 & COUNT_SAMPLING_RATE)) {
	  ...
	  pmc_compose_timestamp(tstamp_str, SRVBUFLEN, &pnat->timestamp_end, TRUE, want_tstamp_since_epoch);
        ...
        if (!have_wtc || (what_to_count_2 & COUNT_EXPORT_PROTO_SOURCEID)) {
          if (want_output & PRINT_OUTPUT_FORMATTED) printf("%-20u  ", acc_elem->primitives.export_proto_sourceid);
          else if (want_output & PRINT_OUTPUT_CSV) printf("%s%u", write_sep(sep_ptr, &count), acc_elem->primitives.export_proto_sourceid);
        }
